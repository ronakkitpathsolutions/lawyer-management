  ...
  const fieldsData = useMemo(
    () => [
      ...
      },
      {
        ...
        placeholder: msg.select("intended closing date"),
        withAsterisk: ["on_or_before", "after", "only_on"].includes(
          intended_closing_date,
        ),
        section: "dates",
        disabled: !["on_or_before", "after", "only_on"].includes(
          intended_closing_date,
        ),
      },
      {
      ...
      },
      {
        ...
        type: "textarea",
        label: "Remarks",
        placeholder: msg.default("remarks"),
        withAsterisk: false,
        focus: false,
        section: "remarks",
      },
  ...
  const [onSubmit, loading, notification] = useAsyncOperation(
    async (payload) => {
      ...
      const hasFiles = fileFields.some(
        (field) => payload[field] && payload[field] instanceof File,
      );
